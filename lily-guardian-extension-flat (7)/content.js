...
(function () {
  ...
  // Wake the free-tier backend as soon as the panel loads so the first analysis
  // doesn't hit the cold-start delay. Throttled in the background worker.
  try {
    chrome.runtime.sendMessage({ type: "WARM_UP" });
  } catch (_e) {
    // ignore — analysis still retries on cold start
  }
  ...
  evidenceBtn.addEventListener("click", async () => {
    ...
    if (response && response.ok) {
      ...
      const ipLabel = m.ip_simulated
        ? '<span class="lg-warn">SIMULADO — IP real não obtido</span>'
        : `real${m.ip_source ? " · via " + escapeHtml(hostOf(m.ip_source)) : ""}`;
      const ipValue = m.ip ? escapeHtml(m.ip) : "—";
      showResult(`<div class="lg-badge ok">Evidência registrada</div>
        ...
        <div class="lg-meta"><b>IP:</b> ${ipValue} (${ipLabel})</div>
