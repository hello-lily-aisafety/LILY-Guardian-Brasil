  ...
  try {
    ...
    if (result && result.ok) {
      ...
      if (!selection) {
        ...
        // One-click flow: load the page selection and analyze it immediately.
...
function renderEvidence(result) {
  ...
  const ipDetail = m.ip_simulated
    ? `<span class="ip-warn">SIMULADO — IP real não obtido</span>`
  ...
  return `
    ...
    <div class="meta-row"><span class="meta-key">IP</span>
      <span class="meta-val">${escapeHtml(m.ip || "—")} (${ipDetail})</span></div>
